...
ALTER TABLE tbl_attach ADD CONSTRAINT tbl_attachv2_pk PRIMARY KEY ( save_file_name );
CREATE TABLE tbl_board (
    bno                         NUMBER(11) NOT NULL,
    title                       VARCHAR2(255),
    content                     CLOB,
    writer                      VARCHAR2(50),
    view_couny                  NUMBER(11),
    reply_count                 NUMBER(11),
    reg_date                    TIMESTAMP,
    update_date                 TIMESTAMP,
    tbl_board_type_board_type   VARCHAR2(20)
)
LOGGING;
...
COMMENT ON COLUMN tbl_board.view_couny IS
    '글조회수카운터';

COMMENT ON COLUMN tbl_board.reply_count IS
    '댓글카운트';

COMMENT ON COLUMN tbl_board.reg_date IS
    '등록일시';
...
COMMENT ON COLUMN tbl_member.user_email IS
    '회원이메일';
...
ALTER TABLE tbl_attach
    ADD CONSTRAINT tbl_attachv2_tbl_board_fk FOREIGN KEY ( bno )
        REFERENCES tbl_board ( bno )
    NOT DEFERRABLE;
ALTER TABLE tbl_board
    ADD CONSTRAINT tbl_board_tbl_board_type_fk FOREIGN KEY ( tbl_board_type_board_type )
