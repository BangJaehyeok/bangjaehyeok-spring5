...
#### 20210722(목) 작업
- 하나의 변수에는 하나의 값만 보관된다.

- 색상정할때 16진수 많이 쓴다.
- 10진수 : 11 8진수 : 011 16진수 : 0x11
- 8진수는 앞에 0을 붙이기로 함. 16진수는 앞에 0x를 붙이기로 했다.
- 010 = 8(8진수니깐)
- 0xA = 10진수로 10.
- 0x5a = 90
- 3.14e7 = 3.14 * 10^7
- -3.14e5 = -3.14 * 10^5
- -2.1e-5 = -2.1*10^-5 = -2.1*1/100000
- char type : 문자타입, 문자열이 아님.
- 문자열은 String
- \n => new line
- \t => tab만큼의 공간
- 정수타입 데이터타입
- byte 1byte , char 2byte, char 2byte(문자저장), short 2byte, int 4byte, long 8byte

-실수타입 데이터타입
- float 4byte, double 8byte;

- type conversion : 타입변환
- 자동타입변환 : 작은 크기의 타입이 큰 크기의 타입으로 배정될때.
- 강제타입변환 : 큰 타입을 작은 타입으로 배정할때, 강제로 타입변환을 해줘야한다.(casting을 이용하여 개발자가 코딩해야한다.)

- 연산자
- 5%3 = 5와3을 나눈 나머지의 값이 구해져서 2가 출력된다. %=나머지 연산자
- m=m+1 m+=1
- m=m-1 m-=1
- m=m*3 m*=3
- m=m%2 m%=2
- 재귀적인 연산을 쓸때만 위 기호들을 쓸수있다. 즉, 자기스스로에게만 연산이 적용될때만 위 기호들을 쓸 수 있다.
- m=n+2 != m+=2
- +: 산술연산자이면서 연결연산자이다. 즉, 문자열변수끼리 묶어줄수있다.
- +는 정수형변수와 문자열변수를 연결할수도있다. 대신 정수형변수는 문자열변수로 바뀌어 연결된 후 출력이 된다.
- =은 대입연산자, == 같다는뜻의 비교연산자.
- 대신 ==은 문자열비교할때 쓰면 안된다. 산술적인것만 가능
- 문자열비교는 .equals("문자열")을 써야한다.
- ex) String str = "BJH";
		if(str.equals("BJH")) {//문자열비교는 이렇게
		
- == <-> !=
- >  <-> <= 크다의 반대는 작거나 같다 이다.
- <  <-> >= 작다의 반대는 크거나 같다이다.
- 비교연산자의 결과값은 True/False로 나타난다.
- 논리연산자 
- && = and 양쪽 다 true면 true, 그외의 경우는 모두 false
- || = or 한쪽이라도 true면 true다.
- ! = not

- 하나의 변수에는 하나의 값만 보관된다.

- 5/0 = x x는 존재하지않는다. 불능. = Divide by Zero(Error)
- Nan(Not a Number). 어떤 숫자를 0으로 나누는 경우는 없어야한다. 어차피 컴터가 못풀긴하지만 아무튼 없어야한다.

- 제어문 : 조건문(if-else),switch, 반복문-while(do-while), for
- if(조건문) 조건문이 true이면 실행문을 실행시킨다. 실행문이 여러개면 {}로 묶어서낸다. 

- Scanner s = new Scanner(System.in);//사용자의 입력을 받아들일 준비
-		String a = s.nextLine();//사용자의 입력을 변수에 저장
-		System.out.println(a);//사용자의 입력이 저장된 변수를 출력
- Scanner를 통해서 사용자의 입력값을 출력한다. 많이 사용.
- int a = s.nextInt();//정수값을 입력할때는 앞에 int하고 nextInt()해줘야함
- String a = s.nextLine();//사용자의 입력을 변수에 저장(문자열을 읽을때는 nextLine())
- int n = Integer.parseInt(a);//문자열을 읽어서 정수타입으로 바꾸는 코딩
- float f = Float.parseFloat(a);//문자열을 실수타입으로 바꾸는 코딩

- 중첩if문. if안에 if를 쓸 수 있다. if안에 else if, else,if 언제든 자유롭게 조건문을 넣어 코딩을 할 수 있다. 
- switch(조건/표현식) {
	case 값/표현식:
	~~;
	break;//케이스 하나가 끝나면 break를 걸어준다. 케이스마다 걸어준다. 
	case 값2/표현식2:
	~~;
	break;
	default: ~~;
...
- L-value = R-value
- L-value는 하나의 변수만 표현
- R-value는 표현식(공식),값,변수
- EX) x(L-value) = 3+2;(R-value)
- 8,16진수는 데이터표현에서 쓰는방법이다. 
================================================
