...
public class AspectAdvice {
	...
	public Object  sessionManager(ProceedingJoinPoint pjp) throws Throwable {
		//board_type변수값을 세션에 저장하려고함. 클라이언트별 세션이 발생됨.
		HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.currentRequestAttributes()).getRequest();
		//일반적인 컨트롤러에서는 매개변수 HttpServletRequest를 사용가능함.위처럼 복잡하지않음
		//컨트롤러 클래스에서 매개변수로 받을값 pageVO(board_type) < pageVO
		PageVO pageVO = null;
		...
		if(request !=null ) {//jsp에서 Get,Post 있을때,
			//세션값을 발생시킴
			HttpSession session = request.getSession();//PC가 스프링프로젝트 접근시 세션객체생성
			...
			if(session.getAttribute("session_board_type") != null) {
				...
				if(pageVO != null) {//Set은 pageVO가 null이 아닐 경우만 실행되도록 설정
				pageVO.setBoard_type(board_type);//검색목표달성 : 여기서 항상 값을 가져가도록 구현됩니다.
				}
			}
			logger.info("디버그: "+(String) session.getAttribute("session_board_type"));
		}
		//Aspect > 포인트컷(Around로 구현) > 조인포인트(메서드) > 매개변수로 구현한 결과를 리턴
