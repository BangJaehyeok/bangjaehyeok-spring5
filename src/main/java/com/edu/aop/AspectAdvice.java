...
/**
 * 이 클래스는 AOP기능 중 @Aspect 와 @ControllerAdvice 로 구현됩니다.
 * @author 방재혁
 *
 */
@Aspect
...
public class AspectAdvice {
	...
	//@Aspect로 AOP기능 추가 = 목적 : board_type값을 항상 가져가도록 처리(세션)
	//세션? 서버-클라이언트 구조상에서 클라이언트가 서버에 접속할때 서버에 발생되는 정보를 세션.(서버에 저장됨)
	//쿠키? 서버-클라이언트 구조상에서 클라이언트가 서버에 접속할때 클라이언트에 발생되는 정보를 쿠기라고함.(Pc에 저장)
	//옛날에는 쿠키로 로그인체크를 했음 -> 보안상 PC에 로그인정보가 저장되기 때문에 매우위험(인터넷광고에서만 사용)
	//Aspect로 AOP를 구현할때는 포인트컷(Advice참견이 실행될 위치)이 필요합니다.
	//@Around(포인트컷 전과 후)=@Before+@After *(...)모든메서드를 의미
	//@Around는 콜백함수의 매개변수로 조인포인트 객체(포인트컷에서 실행되는 메서드들)를 필수로 받습니다.
	//아래 조인포인트(진입점)은 board_delete, board_update 메서드를 실행할때 본인이 작성한 글인지 확인하는 기능추가(단, ROLE_ADMIN은 제외)
	@Around("execution(* com.edu.controller.HomeController.board_delete(..)) ||"
			+ "execution(* com.edu.controller.HomeController.board_update*(..))")
	public Object check_board_crud(ProceedingJoinPoint pjp) throws Throwable {
		...
		if(request != null) {
			...
			//위 메서드의 매개변수를 이용해서 생성된 user_id와 세션userid와 비교+ROLE_USER일때 조건 추가
			if(!user_id.equals(session.getAttribute("session_userid")) 
				&& "ROLE_USER".equals(session.getAttribute("session_levels"))) {
				//메세지를 redirect로 .addFlash~로 변수명을 지정해서 보냈음
				//위 RedirectAttributes를 사용하는 대신 Flash(휘발성객체:1번사용하고 내용없어짐)클래스
		...
		Object result = pjp.proceed();//실제 조인포인트 실행이 여기서됨.
		return result;
	}
	...
	//에러메세지를 개발자가 작성한 jsp화면에 뿌려주는 기능을 추가 prevPage 변수1, exception 변수2
	@ExceptionHandler(Exception.class)
	public ModelAndView errorModelAndView(Exception ex, HttpServletRequest request) {
		//Model(jsp로 data담아서보내주는 객체) + View(페이지 이동할 주소)
		ModelAndView modelAndView = new ModelAndView();
		//이전페이지로 돌아가기용 데이터생성
		String referer = request.getHeader("Referer");//크롬>네트워크>파일>Referer>이전페이지URL
		request.getSession().setAttribute("session_prevPage", referer);//prevPage세션변수 사용가능
