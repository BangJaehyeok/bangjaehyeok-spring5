...
public class AdminController {
	...
	public String board_insert(@RequestParam("file")MultipartFile[] files,BoardVO boardVO) throws Exception {
		//위 메서드의 BoardVO boardVO 파싱 => 내부작동은 다음처럼됨 @RequestParam("title") String title,
		//@RequestParam("writer") String writer, @RequestParam("content") String content...
		//옛날엔 위처럼 일일히 입력해줬는데 지금은 BoardVO boardVO 파싱으로 간단하게 처리한다.
		
		//신규등록이라 기존 첨부파일 불러오는 로직은 필요없음.
		//AttachVO테이블에 가로데이터를 세로데이터로 입력하기 위해서 인서트하는 로직
		//save_file_names[] = ["uuid.jpg","uuid2.jpg"]
		//real_file_names[] = ["슬라이드.jpg","슬라이드2.jpg"]
		String[] save_file_names = new String[files.length];
		String[] real_file_names = new String[files.length];
		int index = 0;//첨부파일 1개 이상일때 반복변수로 사용
			...
			//신규등록 jsp폼에서 보낸 boardVO값에 아래 file에 대한 임시변수값을 저장하는 로직
			boardVO.setSave_file_names(save_file_names);
			boardVO.setReal_file_names(real_file_names);
			boardService.insertBoard(boardVO);//DB에 저장하는 서비스호출
		return "redirect:/admin/board/board_list";//게시판 테러방지용 redirect사용(새로고침시 무한등록방지)
		//게시판 신규등록시 자동으로 page1로 이동하도록 설정
	}
	...
	String[] save_file_names = new String[files.length];
	String[] real_file_names = new String[files.length];
	int idx = 0;//jsp폼에서 보내온 파일에대한 인덱스 초기값 변수
	for(MultipartFile file:files) {//files[0]=file, files[1]
		if(file.getOriginalFilename() != "") {//전송된 첨부파일이 있다면 실행
			...
			for(AttachVO file_name:delFiles) {//기존파일을 가져와서 반복하면서 지우는 로직
				if(idx == sun) {//jsp폼의 파일의 순서와 DB에 저장된 파일의 순서가 일치할때 실행
					File target = new File(commonUtil.getUploadPath(), file_name.getSave_file_name());
					if(target.exists()) {
					target.delete();//물리적인 파일 지우는 명령
					boardDAO.deleteAttach(file_name.getSave_file_name());//DB지우는 부분
					}//if(target.exists())
				}//if(idx == sun)
				sun++;
			}//for-sun
	...
	public String board_update_form(Model model, @RequestParam("bno")Integer bno,@ModelAttribute("pageVO") PageVO pageVO) throws Exception {
		//첨부파일용 save_file_names, real_file_names 2개 배열값을 구해서 boardVO에 입력필요
		BoardVO boardVO = new BoardVO();
		boardVO = boardService.readBoard(bno);
		//여기서 첨부파일 배열을 추가(아래)
		List<AttachVO> listAttachVO = boardService.readAttach(bno);//세로리스트데이터를받음
		...
		boardVO.setSave_file_names(save_file_names);
		boardVO.setReal_file_names(real_file_names);
		model.addAttribute("boardVO", boardVO);//1개코드저장
		
		return "admin/board/board_update";//.jsp생략
	}
	
	//게시물 삭제는 URL쿼리스트링으로 접근하지 않고, post방식으로 처리한다.
	...
	public String board_delete(@RequestParam("bno")Integer bno,PageVO pageVO) throws Exception {
		...
		//DB테이블 삭제하기 이후, 첨부파일부터 있으면 삭제처리 후 자바에서 파일핸들링처리
		//기존 등록된 첨부파일 폴더에서 삭제할 UUID(고유한 식별자 생성 클래스)이름을 추출합니다.(아래)
		List<AttachVO> delFiles = boardService.readAttach(bno);
		//해당 게시물의 모든 첨부파일 delFiles에 임시로 담아 놓습니다.
		
		boardService.deleteBoard(bno);//첨부파일 테이블삭제 후 게시물 테이블 삭제
		//물리적으로 파일삭제 처리시작, 향상된 for문 사용
	...
	//게시물 상세보기 폼으로 접근하지 않고 URL쿼리스트링으로 접근(=GET으로 접근)
	...
	public String board_view(@RequestParam("bno")Integer bno,@ModelAttribute("pageVO")PageVO pageVO, Model model) throws Exception {
		...
		//개발자가 만든 클래스형 객체 boardVO는 개발자가 만든 메서드를 사용
		//반면, List<AttachVO> List클래스형 객체. files는 내장형 메서드 = .size()
		...
		int cnt = 0;
		for(AttachVO file_name:files) { //files 다수레코드에서 1개의 레코드씩 추출
			save_file_names[cnt] = file_name.getSave_file_name();
			real_file_names[cnt] = file_name.getReal_file_name();
			cnt = cnt + 1; //cnt++;
		}
		//위 for문은 세로데이터(다수레코드)를 가로데이터(1레코드면, 배열)에 담아서 1개 레코드 boardVO에 담는게 목적
		
		boardVO.setSave_file_names(save_file_names);//파싱한 결과 Set, 다운로드 로직 필요
		boardVO.setReal_file_names(real_file_names);//boardVO에 Set, 화면에 보일때
		model.addAttribute("boardVO", boardVO); //게시물+첨부파일명2개
		//업로드한 파일이 이미지인지 아닌지 확인하는 용도의 데이터입니다.(아래) (이미지일때 미리보기 img태그를 사용)
	...
	//bbs_type_insert.jsp의 입력폼에서 전송된 값을 boardTypeVO 자동으로 담아서 {구현} 
	//단,자동으로 값이 바인딩되려면, 폼의name과, VO멤버변수명과 동일해야함
	...
	public String insertBoardType(BoardTypeVO boardTypeVO) throws Exception {//입력처리1
		...
		//redirect(뒤로가기 데이터사라짐)는 절대경로, forward:이동이 가능(뒤로가기 데이터가 살아있음-디도스같은 공격에 취약)
		//쇼핑몰 결제화면을 처리 후 뒤로가기를 누르면, redirect는 데이터가 없어져서 재결제 불가
		//forward로 결제화면을 처리 후 뒤로가기를 누르면, 재결제가 발생한다. 이러면 안되서 forward를 사용안함.
	}
	...
	public String updateBoardTypeForm(@RequestParam("board_type")String board_type, Model model) throws Exception {//수정폼1
		...
		//서식 model.add~("jsp변수로 담아서 view화면으로 보냄","서비스에서 쿼리실행한 데이터객체");
	...
	public String updateBoardType(BoardTypeVO boardTypeVO) throws Exception {//수정처리1
		...
		return "redirect:/admin/bbs_type/bbs_type_update?board_type="+boardTypeVO.getBoard_type();//수정한 이후 다시 GET방식으로 수정폼이동
	}
	...
	public String deleteBoardType(@RequestParam("board_type")String board_type) throws Exception {//삭제처리1
		boardTypeService.deleteBoardType(board_type);//삭제서비스 호출(실행)
		return "redirect:/admin/bbs_type/bbs_type_list";
	}
	//=========================================================================================
	...
	public String deleteMember(HttpServletRequest request,MemberVO memberVO) throws Exception {
		...
		//DB테이블 삭제 후 회원 프로필이미지가 exist()==true면 삭제하는 로직 추가
	...
	//메인페이지 또는 대시보드에 최신 테이블리스트를 출력하는 방법2가지(위 model사용)
	//아래, <c:import방식 : 최신게시물용도로 사용 / 페이지 안에서 컴파일된 다른 페이지를 불러올수있음
	@RequestMapping(value="/admin/latest/latest_board", method=RequestMethod.GET)
	public String latest_board(@RequestParam(value="board_name",required=false) String board_name, 
			@RequestParam(value="board_type", required=false) String board_type,Model model) throws Exception {
