...
//게시물 수정 처리 POST 추가
	...
	public String board_update(HttpServletRequest request,@RequestParam("file")MultipartFile[] files,
			PageVO pageVO,BoardVO boardVO,RedirectAttributes rdat) throws Exception {
		/*아래 기능을 AOP로 구현합니다. 1
		//로그인한 세션ID와 게시물의 boardVO.writer사용자와 비교해서 같으면 계속진행, 다르면 멈춤.
		HttpSession session = request.getSession();
		if(!boardVO.getWriter().equals(session.getAttribute("session_userid"))) {
			
			rdat.addFlashAttribute("msgError", "게시물은 본인글만 수정 가능합니다.");
			//request객체에 이전페이지 URL("Referer")로 존재하고, 이 URL을 이전페이지 이동으로 사용(아래)
			return "redirect:"+request.getHeader("Referer");
		}//else로 묶을 필요가 없다. 위 return을 만나면, 이후 실행이 안되고 메서드가 종료됨.
		*/
	...
	public String board_update_form(HttpServletRequest request,@RequestParam("bno")Integer bno,
		@ModelAttribute("pageVO")PageVO pageVO,Model model, RedirectAttributes rdat) throws Exception {			
		//1개의 레코드만 서비스로 호출해서 모델로 보내줌 첨부파일은 세로데이터를 가로데이터로 변경 후 boardVO에 담아서 전송해야함.
		...
		*/
		...
		int index = 0;
		String[] save_file_names = new String[fileList.size()];//각각 배열을 만들어줌
	...
	public String board_delete(HttpServletRequest request,@RequestParam("bno")Integer bno, 
			RedirectAttributes rdat, PageVO pageVO) throws Exception {
		/*아래 공통기능을 AOP로 구현합니다.3
		...
		*/
		...
		for(AttachVO file:delFiles) {//향상된 for문에서는 조건문필요없다.			
			...
			if(target.exists()) {//타겟폴더의 파일이 존재하면 삭제구현(아래)
				target.delete();//물리적인 UUID파일명의 파일삭제처리.
			}
		}
	...
	public String board_view(Model model,@RequestParam("bno")Integer bno,@ModelAttribute("pageVO")PageVO pageVO) throws Exception {
		...
		BoardVO boardVO = boardService.readBoard(bno);//1개의 레코드가 입력됨.
		boardVO.setSave_file_names(save_file_names);
		boardVO.setReal_file_names(real_file_names);
		//DB테이블 데이터 가져오기
		model.addAttribute("boardVO", boardVO);
	...
	//네아로 로그인 때문에 LoginController클래스로 따로 분리해서 사용. 아래는 주석처리.
