...
public class HomeController {
	...
	//게시물 수정 처리 POST 추가
		...
		public String board_update(HttpServletRequest request,@RequestParam("file")MultipartFile[] files,
			...
			if(!boardVO.getWriter().equals(session.getAttribute("session_userid"))) {
				...
				return "redirect:/home/board/board_view?bno="
						+boardVO.getBno()+"&page="+pageVO.getPage();//본인 ID의 글이 아닐때 뷰페이지로 이동
	...
	public String board_update_form(HttpServletRequest request,@RequestParam("bno")Integer bno,
			@ModelAttribute("pageVO")PageVO pageVO,Model model, RedirectAttributes rdat)
	throws Exception {			
