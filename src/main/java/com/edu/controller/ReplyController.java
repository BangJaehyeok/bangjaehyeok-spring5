...
public class ReplyController {
	...
	@RequestMapping(value="/reply/reply_list/{bno}/{page}", 
			method=RequestMethod.POST)//POST사용이유 : 현재 도메인에서만 사용가능하도록 하기위해서
	public ResponseEntity<Map<String,Object>> reply_list(@PathVariable("bno")Integer bno,
		...
		//URL주소가 아닌 Json데이터형으로 자료를 반환합니다. 
		...
		//아래의 Json데이터 형태는 일반 컨트롤러에서 사용했던 model사용해서 ("변수명",객체내용) 전송한 방식과 동일
		resultMap.put("replyList", dummyMapList);
		//------------------------------------------------------------------------
		
		//result객체를 만든 목적 : RestApi클라이언트(jsp쪽)으로 resultMap객체를 보낼때 상태값도 같이 보내기 위해
		result = new ResponseEntity<Map<String,Object>>(resultMap,HttpStatus.OK);
		//객체를 2개 이상 보내게 되는 상황일때, Json데이터 형태(key:value)로 만들어서 보냅니다.
		//===========================================================================
		} catch(Exception e) {
		 ...
		 *  {"rno":"1","reply_text":"댓글테스트1","replyer":"admin","bno":59} == Map<String,Object>
