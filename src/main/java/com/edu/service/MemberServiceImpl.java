...
//헤로쿠클라우드에 30분 휴면상태 깨우는 기능 추가 ->root-context에서 지정한 스케쥴러에서 20분간격으로 호출됨
	public void herokuJobMethod() throws Exception {
		//월-금 오전8시부터 오후11시까지(미국시간 0시-14시) 헤로쿠 앱에 20분 간격으로 접근
		//헤로쿠 컨테이너가 러닝할 수 있는 무료 시간 1달 700시간. 시간넘으면 먹통
		String urlStr = "https://bangjaehyeok-spring5.herokuapp.com/";
		URL url = new URL(urlStr);
		HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();//접속객체 만들기
		urlConnection.setUseCaches(false);//접속시 캐시사용없이 무조건 새로고침하겠다고 명시, 컨테이너가 계속 떠있습니다.
