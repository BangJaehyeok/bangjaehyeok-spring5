...
$(document).ready(function(){
	...
	//alert(current);//url값을 확인해서 파싱(분해)하기위해
	var current2 = current.split("/")[2];//위 current변수값을 /로 분해해서 배열로 만든 후 
	//그 중 0,1,2 즉, 3번째 배열값을 추출
	//alert(current2);//member,bbs_type,board,undefined
	//제이쿼리의 반복문  each : 영역.each(구현내용)
	$(".nav-treeview li a").each(function(){
		if($(this).attr('href').indexOf(current2) != -1) {//URL(href)에서 current2와 같은 값이 있다면
			//indexOf는 배열로서 0부터 시작. != -1이라는건 같은값이 있다면 이란것.
			if(current2 != "board"){//게시물관리 메뉴만 제외하고, 다른메뉴에 적용
