...
<div id="container">
        ...
        <div class="box_inner">
            ...
            <ul class="page_menu clear">            
            	<c:forEach var="boardTypeVO" items="${listBoardTypeVO}">
	            	<li><a href="/home/board/board_list?board_type=${boardTypeVO.board_type}&search_keyword=" 
	            	class="${boardTypeVO.board_type==session_board_type?'on':''}">${boardTypeVO.board_name}</a></li>	                
            	</c:forEach>                
            </ul>
        </div>
    </div>	
    ...
    <div class="bodytext_area box_inner">
        ...
        <table class="bbsListTbl" summary="번호,제목,조회수,작성일 등을 제공하는 표">
            ...
            <tbody>
            	<c:forEach var="boardVO" items="${boardList}" varStatus="status">
	            	<tr>
	                    <td>
	                    <!-- 전체게시물개수-(현재페이지X1페이지당 보여줄개수)+1페이지당 보여줄개수 -현재인덱스(위부터 0으로시작) -->
        ...
        <!-- //페이징처리영역 -->
        <p class="btn_line">
        <!-- 등록버튼은 로그인한 사용자만 보이도록 설정 -->
        <c:if test="${session_enabled}">
        	<!-- 게시판이 공지사항일때는 관리자만 사용하도록 설정, 공지사항 외에는 로그인사용자 모두 글쓰기 가능 -->
