 ...
 * 장점: 메모리 낭비를 방지하는 효과
 * 예, DB접속객체를 예를 들면
 * Connection connection1 = dataSource.getConnection();// new키워드를 사용하지않는다.
 * Connection 객체가 생성되어있으면,
 * Connection connection2 = dataSource.getConnection();//싱글톤 객체
 * Connection connection3 = dataSource.getConnection();//싱글톤 객체
 * connection1==connection2==connection3 다 같은 동일객체로 같은 메모리를 사용한다. 
 * 즉, 1개의 객체(인스턴스)만 생성됩니다.
